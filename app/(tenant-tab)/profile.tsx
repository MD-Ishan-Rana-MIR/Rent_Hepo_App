import { password, profile, updateIcon } from '@/lib/icon';
...
const Profile = () => {
    ...
    // 2. Profile API Query
    const { data, isLoading: profileLoading } = useUserProfileQuery({});
            ...
            >
                ...
                <View style={tw`items-center mb-8`}>
                    <View style={tw`relative`}>
                        <View style={tw`w-32 h-32 rounded-full border-4 border-primaryBorder p-1 items-center justify-center`}>
                            {profileLoading ? (
                                <ActivityIndicator size="small" color={tw.color('primaryBorder')} />
                            ) : (
                                <Image
                                    source={{ uri: data?.data?.user?.avatar_url || 'https://via.placeholder.com/150' }}
                        ...
                        <TouchableOpacity
                            style={tw`absolute bottom-2 right-4 bg-primaryText w-4 h-4 rounded-full items-center justify-center border-2 border-white`}
                        >
                ...
                <View style={tw`bg-white border border-primaryBorder/10 rounded-[24px] px-5 shadow-sm mb-6`}>
                    <MenuItem
                        icon={<SvgXml xml={updateIcon} width="18" height="18" />}
                        title="Update Profile"
                        onPress={() => router.push("/(update-profile)/user-update-profile")}
                    />

                    <MenuItem
                        icon={<SvgXml xml={password} width="18" height="18" />}
                        title="Change Password"
                        onPress={() => router.push("/(password-update)/UserPasswordChange")}
                        isLast={true}
                    />
